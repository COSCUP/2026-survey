...
Apps Script 每 15 分鐘比對最新完整快照
        ↓
只把尚未存在的報名新增至私人 Google Sheet「報名資料」
...
若 CSV 沒有這類欄位，則使用姓名、Email、付款時間與取票時間組成比對鍵。只有尚未存在的報名
會新增到「報名資料」最後一列，既有列不會被清空、覆寫或重複新增。
...
差異匯入採「只新增、不覆寫」：已存在報名後來修改問卷答案或新增取消時間時，不會改寫舊列。
CSV 與 Sheet 的欄位或順序不同時，程式會停止匯入，避免資料落入錯誤欄位。
